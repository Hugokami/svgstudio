...
## 2024-05-25 - Caching dynamically evaluated math expressions inside animation loops
**Learning:** In GSAP timeline evaluation callbacks (e.g., `gsap.ticker.add`), calling `new Function(...)` repeatedly to parse `data-expr-*` attributes generates significant JIT compilation overhead on every frame, which hurts frontend performance, especially when scrubbing timelines or applying multiple animated mathematical filters.
**Action:** Always maintain a shared `Map` (like `window.expressionCache`) to cache constructed functions based on their string expressions.
